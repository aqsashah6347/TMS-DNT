...
/**
 * Cursor-reactive fluid/splash effect, scoped to a container element
 * (not the whole viewport). Pass a ref to the element you want the
 * effect to track mouse movement within and fill visually.
 */
function TeamFluidCursor({
  containerRef,
  SIM_RESOLUTION = 96,
  DYE_RESOLUTION = 720,
  ...
  useEffect(() => {
    ...
    function handleTouchStart(e) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const { x, y } = getRelativePos(touches[i].clientX, touches[i].clientY);
...
function handleTouchEnd(e) {
    ...
    // Event listeners ko container par attach karna
    ...
    // WebGL animation loop shuru karna
    updateFrame();

    // Cleanup function: Jab component band ho to memory clear karna
    return () => {
      isActive = false;
  ...
  return (
    <canvas
      ...
      style={{
        ...
        width: '100%',
        height: '100%',
        pointerEvents: 'none', // Taaki text ya buttons par click ho sake
        zIndex: 1
      }}
    />
  );
...
// Yeh sabse zaroori hai taaki doosri files isse use kar sakein
export default TeamFluidCursor;
