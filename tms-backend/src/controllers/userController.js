...
async function getAllUsers(req, res, next) {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT id, name, email, role, status FROM tms_users ORDER BY name ASC");
...
async function updateUser(req, res, next) {
  try {
    ...
    if (role !== undefined) { request.input("role", sql.NVarChar, role); setClauses.push("role = @role"); }
